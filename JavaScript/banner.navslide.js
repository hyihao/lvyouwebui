$(function () {
    (function ($) {
        $.fn.NewsNavSlide = function (options) {
            ...
            function loading() {
                ld.css({ "height": "0", "height": "2px", "position": "absolute", "left": "0", "bottom": "0", "background": "#3586F2", "z-index": "10" });
                ld.animate({ "width": "100%" }, 9000).animate({ "width": 0 }, 0);
            }
            return this.each(function () {
                auto(); navitem.hover(function () {
                    _stop(); var i = navitem.index(this);
                    if (/nownav/.test($(this).attr('class'))) {
                        return false;
                    }
                    if (newtimer) clearTimeout(newtimer); newtimer = setTimeout(function () { _stop(); ld.stop(true, true).animate({ "width": 0 }, 0); anim(i, this); }, 250);
                }, auto); $('#next').on('click', next); $('#prev').on('click', prev);
            });
        };
    })(jQuery);
    $("#slide").NewsNavSlide();
